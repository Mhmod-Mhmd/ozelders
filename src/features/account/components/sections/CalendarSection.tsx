import { useTranslation } from 'react-i18next';
import { Check, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { ErrorState } from '@/components/ui/ErrorState';
import { useCalendar, useSetCalendar } from '../../hooks/useAccount';

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-7 w-7 shrink-0" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.27-4.74 3.27-8.1Z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.1a6.6 6.6 0 0 1 0-4.2V7.06H2.18a11 11 0 0 0 0 9.88l3.66-2.84Z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.46 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"
      />
    </svg>
  );
}

/** Google Calendar connection panel. */
export function CalendarSection() {
  const { t } = useTranslation();
  const { data, isLoading, isError, refetch } = useCalendar();
  const setCalendar = useSetCalendar();

  if (isLoading) {
    return (
      <div className="flex items-center gap-3 py-10 text-gray-500">
        <Loader2 className="h-5 w-5 animate-spin" />
        {t('common.loading')}
      </div>
    );
  }
  if (isError || !data) {
    return <ErrorState onRetry={() => refetch()} />;
  }

  const connected = data.connected;

  return (
    <div className="flex max-w-2xl flex-col gap-6">
      <h1 className="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">
        {t('settings.calendar.title')}
      </h1>

      <div className="flex items-start gap-3">
        <GoogleIcon />
        <p className="text-sm text-gray-700">
          {t('settings.calendar.description')}
        </p>
      </div>

      <Button
        type="button"
        variant={connected ? 'outline' : 'primary'}
        size="lg"
        disabled={setCalendar.isPending}
        onClick={() => setCalendar.mutate(!connected)}
        className="w-full gap-2 sm:w-fit"
      >
        {setCalendar.isPending && <Loader2 className="h-4 w-4 animate-spin" />}
        {connected && !setCalendar.isPending && <Check className="h-4 w-4" />}
        {connected
          ? t('settings.calendar.disconnect')
          : t('settings.calendar.connect')}
      </Button>
    </div>
  );
}
